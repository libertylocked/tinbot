...
def get_fb_access_token(email_and_password):
...
def get_access_token():
    """Gets FB access token, from either a file, or by logging in"""
    # Attempt to use a saved token
    fb_token = get_fb_access_token_from_file()
    if fb_token is None:
        print "Invalid auth token! Login required"
        email_and_password = get_fb_credentials_from_input()
